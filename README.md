...
The field strength stored with the grid, and hence `bfield.dat`, is evaluated with the same
perturbations as the field-line tracing: the single perturbation `A` for the time-independent
method, `A1`, `A2`, `A3` for the time-dependent method, or `A1` plus the `A_list` perturbations
for `flag_f_modificator = 2` or `3`. The psi envelope of `flag_f_modificator = 1` and `2` is not
...
| `target_generation/target_generation.py`, `target_generation/read_and_plot_targets.py` | Divertor target and vessel generation in the Kisslinger format for EMC3-EIRENE, and plotting of the generated surfaces |
